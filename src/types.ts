...
export interface HydratedRanking extends ScoreBreakdown {
  name: string;
  headline: string;
  location: string;
  years_of_experience: number;
  current_company: string;
  expected_salary_range: { min: number; max: number };
  notice_period_days: number;
...
export interface SourcingAnalytics {
  ...
  behaviorDistribution: Array<{ name: string; value: number }>;
  educationDistribution: Array<{ name: string; value: number }>;
  pipelineFunnel: Array<{ step: string; count: number }>;
